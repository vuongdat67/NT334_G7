...
class LLMClient:
    ...
    def _apply_rpm_delay(self):
        # User defined limits: gemini-3.1-flash-lite: 15 RPM, gemma-4-31b-it: 30 RPM
        rpm_map = {
            "gemini-3.1-flash-lite": 15,
            "gemma-4-31b-it": 30
        }
        
        limit = rpm_map.get(self.model)
        if limit:
            delay = 60.0 / limit
    ...
    def ping(self, prompt: str = "Reply with exactly: HEALTHY", retries: int = 3) -> str:
        last_err = None
    ...
    def _extract_json(text: str) -> Optional[Dict[str, Any]]:
        ...
        # Remove reasoning blocks like <thought> or <think> generated by some models
        stripped = re.sub(r'<thought>.*?</thought>\s*', '', stripped, flags=re.DOTALL)
    ...
    def triage_once(self, prompt: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            ...
            return {
                "suspicious_processes": [],
                "api_error": f"Failed after {max_retries} attempts: {last_error}",
            }
